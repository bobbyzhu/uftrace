...
void mcount_arch_find_module(struct mcount_dynamic_info *mdi)
{
	...
	for (i = 0; i < ehdr.e_shnum; i++, pos += ehdr.e_shentsize) {
		...
		pread(fd, adi->xrmap, shdr.sh_size, shdr.sh_offset);
